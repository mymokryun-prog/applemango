...
const authenticateToken = (req: AuthRequest, res: Response, next: NextFunction) => {
  ...
  // For development: allow requests without token with default user
  if (!token) {
    req.user = { userId: 'user-minsu' };
    return next();
  }
...
const messages: any[] = [
  ...
  {
    id: 'msg-init-2',
    senderId: 'friend-jiwoo',
    senderName: '지우 (애망러브)',
    senderAvatar: '🍎',
    senderColor: '#EF4444',
    text: '헤이 다들 어디야? 나 홍대입구 경의선숲길 지나고 있어!',
    timestamp: new Date(Date.now() - 1800000).toISOString(),
  },
  {
    id: 'msg-init-3',
    senderId: 'friend-taegon',
    senderName: '태건 (초코망고)',
    senderAvatar: '🥭',
    senderColor: '#F97316',
    text: '나는 합정에서 출발해 걸어가는 중! 10분 정도 걸릴 듯.',
    timestamp: new Date(Date.now() - 900000).toISOString(),
  }
];

// Friends lists with routes centered on lively Hongdae streets
...
const notifications: any[] = [
  ...
  },
  {
    ...
    type: 'arrival',
    title: '친구 위치 접근 알림',
    message: '태건 님이 약속 장소 반경 500m 이내로 들어왔습니다.',
    timestamp: new Date(Date.now() - 600000).toISOString(),
    read: false,
  }
];

// Automatically update friend locations occasionally to simulate real-time movement
const dbRooms: Record<string, any> = {
  ...
  'room-family': {
    ...
    messages: [
      ...
      {
        id: 'msg-fam-init-2',
        senderId: 'family-mother',
        senderName: '엄마 (엄망고)',
        senderAvatar: '👵',
        senderColor: '#EC4899',
        text: '민수야 오늘 저녁에 망원동 한강공원에서 맛있는 과일 디저트 먹을 계획인데 어떠니? 🍉',
        timestamp: new Date(Date.now() - 1800000).toISOString(),
      },
      {
        id: 'msg-fam-init-3',
        senderId: 'family-father',
        senderName: '아빠 (애플파파)',
        senderAvatar: '👴',
        senderColor: '#3B82F6',
        text: '엄마 생각에 한 표! 아빠 자전거 타고 서강대교 지나서 안전하게 퇴근하는 중이다.',
        timestamp: new Date(Date.now() - 900000).toISOString(),
      }
    ],
    friends: {
    ...
    appointments: [
      {
        id: 'promise-family-1',
        title: '🏠 주말 마포 한강공원 망원지구 텐트치기',
        placeName: '망원 한강공원 잔디마당',
        lat: 37.5560,
        lng: 126.9015,
        datetime: '이번 주 토요일 오후 2시',
        creatorName: '엄마 (엄망고)',
        attendees: ['엄마 (엄망고)', '나 (민수)'],
        votes: {
          'family-mother': 'yes',
          'user-minsu': 'yes',
          'family-father': 'maybe',
          'family-brother': 'maybe'
        }
      }
    ],
    notifications: [
      {
        ...
        type: 'promise',
        title: '새 위시 일정 소집',
        message: '엄마 님이 주말 한강공원 텐트치기 소집 깃발을 꽂아놓았습니다.',
        timestamp: new Date(Date.now() - 1700000).toISOString(),
        read: false,
      }
    ]
  ...
  'room-work': {
    ...
    messages: [
      ...
      {
        id: 'msg-work-init-2',
        senderId: 'work-manager',
        senderName: '김팀장 (버드리더)',
        senderAvatar: '🦁',
        senderColor: '#EF4444',
        text: '여러분, 오늘 4시 서교동 쪽에서 파트너 미팅 끝나고 약식 피드백 및 커피 타임 갖겠습니다. 다들 근방이시죠?',
        timestamp: new Date(Date.now() - 1800000).toISOString(),
      },
      {
        id: 'msg-work-init-3',
        senderId: 'work-assistant',
        senderName: '박대리 (일잘러)',
        senderAvatar: '🦊',
        senderColor: '#F59E0B',
        text: '네! 저는 합정역 인근 미팅 끝나고 바로 홍대입구 방면으로 기획서 검토하면서 가고 있습니다!',
        timestamp: new Date(Date.now() - 900000).toISOString(),
      }
    ],
    friends: {
    ...
    appointments: [
      {
        id: 'promise-work-1',
        title: '👔 신규 서비스 아이디어 조율 티타임',
        placeName: '서교동 커피 딜라이트 2층',
        lat: 37.5548,
        lng: 126.9221,
        datetime: '오늘 오후 4시',
        creatorName: '김팀장 (버드리더)',
        attendees: ['김팀장 (버드리더)', '나 (민수)'],
        votes: {
          'work-manager': 'yes',
          'user-minsu': 'yes',
          'work-assistant': 'maybe',
          'work-intern': 'yes'
        }
      }
    ],
    notifications: [
      {
        ...
        type: 'promise',
        title: '티타임 소집 깃발 알림',
        message: '김팀장 님이 서교동 딜라이트 카페 자리에 깃발을 꽂았습니다! 🍎🥭',
        timestamp: new Date(Date.now() - 1700000).toISOString(),
        read: false,
      }
    ]
  ...
  'room-care': {
    ...
    messages: [
      ...
      {
        id: 'msg-care-init-2',
        senderId: 'care-grandfather',
        senderName: '할아버지 (행복망고)',
        senderAvatar: '👴',
        senderColor: '#374151',
        text: '민수야, 이 늙은이는 오늘 아침 일찍 홍대 앞 노인복지관 다녀오는 길이다. 날씨가 참 상쾌하구나!',
        timestamp: new Date(Date.now() - 1800000).toISOString(),
      },
      {
        id: 'msg-care-init-3',
        senderId: 'care-grandmother',
        senderName: '할머니 (정정애플)',
        senderAvatar: '👵',
        senderColor: '#EC4899',
        text: '할머니는 망원시장 앞 정형외과 물리치료 끝나고 집으로 들어가는 중이란다 👵',
        timestamp: new Date(Date.now() - 900000).toISOString(),
      }
    ],
    friends: {
    ...
    appointments: [
      {
        id: 'promise-care-1',
        title: '👵 할머니 망원동 정형외과 모셔다 드리기',
        placeName: '효산 정형외과의원',
        lat: 37.5535,
        lng: 126.9110,
        datetime: '다음 주 목요일 오전 10시',
        creatorName: '할머니 (정정애플)',
        attendees: ['할머니 (정정애플)', '나 (민수)'],
        votes: {
          'care-grandmother': 'yes',
          'user-minsu': 'yes',
          'care-grandfather': 'maybe'
        }
      }
    ],
    notifications: [
      {
        ...
        type: 'arrival',
        title: '할아버지 이동 감지',
        message: '할아버지 님이 홍대 앞 노인복지관에 안전하게 도착했습니다. 👴',
        timestamp: new Date(Date.now() - 1700000).toISOString(),
        read: false,
      }
    ]
...
function syncUserMinsu(lat: number, lng: number, statusMsg?: string) {
  Object.keys(dbRooms).forEach(rId => {
    const r = dbRooms[rId];
    if (r.friends['user-minsu']) {
      r.friends['user-minsu'].lat = lat;
      r.friends['user-minsu'].lng = lng;
      if (statusMsg !== undefined) {
        r.friends['user-minsu'].statusMsg = statusMsg;
      }
      r.friends['user-minsu'].updatedAt = new Date().toISOString();
    }
  });
}
...
function applyFriendLocationUpdate(
  ...
  if (friendId === 'user-minsu') {
    syncUserMinsu(lat, lng, statusMsg);
  }
...
async function startServer() {
  ...
  app.post('/api/rooms', validateRequest(RoomSchema), tryCatch(async (req: AuthRequest, res: Response) => {
    ...
    const userMinsuTemplate = dbRooms['room-friends'].friends['user-minsu'];
    const customFriends: Record<string, any> = {
      'user-minsu': { ...userMinsuTemplate }
    };
  ...
  app.post('/api/friends/profile', validateRequest(ProfileSchema), (req: AuthRequest, res: Response) => {
    const { phone, realName, alias } = req.body;
    
    // Update user-minsu's state details globally
    Object.keys(dbRooms).forEach(roomId => {
      const r = dbRooms[roomId];
      if (r.friends['user-minsu']) {
        r.friends['user-minsu'].phone = phone || r.friends['user-minsu'].phone || '010-1234-5678';
        r.friends['user-minsu'].realName = realName || r.friends['user-minsu'].realName || '김민수';
        r.friends['user-minsu'].alias = alias || r.friends['user-minsu'].alias || '애망민수';
        
        // Construct visual presentation name
        if (alias) {
          r.friends['user-minsu'].name = `${alias} (${realName || '민수'})`;
        } else if (realName) {
          r.friends['user-minsu'].name = `나 (${realName})`;
        }
        r.friends['user-minsu'].updatedAt = new Date().toISOString();
      }
    });
